...
interface Championship {
  name: string; classification: Team[]; matches: Match[];
...
export default function MunicipalPage() {
  ...
  useEffect(() => {
    fetch("/api/municipal")
      ...
      .then((d) => {
        setData(d);
        if (d[0]?.totalRounds) setSelectedRound(d[0].totalRounds);
      })
      .catch(() => {})
    ...
    <div className="mx-auto max-w-[1240px] px-4 py-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-text-muted">Santana de Parnaiba | Atualizado: {new Date(champ.updatedAt).toLocaleDateString("pt-BR")}</p>
        </div>
      </div>
        ...
        <div className="bg-card-bg rounded-lg border border-border-custom">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {champ.classification.map((t) => {
                  let border = "";
                  if (t.pos <= 4) border = "border-l-2 border-l-green";
                  else if (t.pos >= champ.classification.length - 1) border = "border-l-2 border-l-red";

                  return (
                    <tr key={t.pos} className={`border-b border-border-light last:border-0 hover:bg-card-hover ${border}`}>
        ...
        <div className="bg-card-bg rounded-lg border border-border-custom">
          ...
          <div className="px-4 py-3 border-b border-border-custom">
            <div className="flex items-center justify-between bg-body rounded-lg px-3 py-2">
              <button
                onClick={() => setSelectedRound((r) => Math.max(rounds[0], r - 1))}
              ...
              <button
                onClick={() => setSelectedRound((r) => Math.min(rounds[rounds.length - 1], r + 1))}
                  ...
                  <div key={i} className="px-4 py-3">
                    {m.date && (
                      <div className="text-[10px] text-text-muted text-center mb-2">
                        {m.date} {m.time && `- ${m.time}`} {m.status && `| ${m.status}`}
                      </div>
                    )}
