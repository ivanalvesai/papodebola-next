...
// TEMPORÁRIA: cria/atualiza a Página do CMS "jogos-de-hoje-futebol" (collection pages) com
// o conteúdo do mockup (todayGames + campeonatos + FAQ + rodapé + SEO). Idempotente por slug.
// Protegida por REVALIDATION_SECRET. Rodar UMA vez (depois da migração do bloco todayGames).
// REMOVER depois.  GET/POST /api/seed-agenda-page?secret=XXX
export const dynamic = "force-dynamic";

/* ---------- helpers lexical ---------- */
/* eslint-disable @typescript-eslint/no-explicit-any */
const t = (text: string, format = 0): any => ({ type: "text", version: 1, detail: 0, format, mode: "normal", style: "", text });
const b = (text: string): any => t(text, 1); // bold
...
const listBlock = (...items: any[][]) => ({ blockType: "list", items: items.map((nodes) => ({ content: rich(...nodes) })) });
const note = (text: string) => ({ blockType: "note", text });

const LAYOUT: any[] = [
  // 1) Widget dinâmico: todos os campeonatos (com empty-state). Pra reordenar por liga,
  //    troque por vários blocos "todayGames" com league = copa-do-mundo / brasileirao-serie-a / etc.
  {
  ...
  // 2) Principais Campeonatos de Futebol
  heading("Principais Campeonatos de Futebol", "h2"),
  listBlock(
    [a("Copa do Mundo 2026", "/futebol/copa-do-mundo")],
    [a("Brasileirão Série A", "/futebol/brasileirao-serie-a")],
    [a("Brasileirão Série B", "/futebol/brasileirao-serie-b")],
    [a("Copa do Brasil", "/futebol/copa-do-brasil")],
    [a("Copa Libertadores", "/futebol/libertadores")],
    [a("Champions League", "/futebol/champions-league")]
  ),
