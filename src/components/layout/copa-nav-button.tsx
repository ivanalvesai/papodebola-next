...
export function CopaNavButton({
  ...
  return (
    <Link
      ...
      className={`pdb-copa-btn group relative inline-flex items-center gap-1.5 overflow-hidden whitespace-nowrap rounded-full font-extrabold uppercase tracking-wide text-[#0a4d2e] shadow-[0_2px_8px_rgba(201,162,39,0.45)] ring-1 ring-[#8a6c1b]/40 transition-transform duration-200 hover:scale-[1.04] ${
        full ? "w-full justify-center px-4 py-2.5 text-sm" : "px-3 py-1.5 text-[12px]"
      }`}
      style={{ background: WC_GOLD }}
    >
      <Trophy className={full ? "h-4 w-4 shrink-0" : "h-3.5 w-3.5 shrink-0"} />
